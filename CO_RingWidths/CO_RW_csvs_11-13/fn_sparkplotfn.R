###########################################
#        Making Sparkline figures         #
###########################################

#makes stacked sparkline figures ala Tufte
# stacks vertically, layering by "type"
# legend appears at top right

# This is a general function, whose code has been directly coopted for all of my sparkplot figures.
# included in the end of this function is a lot of commented out code that was me messing around with 
# this code to make pretty figures. All of the pertinent code has been copied into specific files
# So this is all commented out

##INPUTS: years= vector of x variable for plot
      #   names= vector of names differentiating each tree
      #   data= vector of y variables to be plotted
      #   types= vector of types to layer by (if not desired, insert vector of 1s)
      #   legend= default TRUE, whether to print legend or not

# testing data
#years <- rw.clim.df.narm$Year
#names <- rw.clim.df.narm$Tree
#data <- rw.clim.df.narm$value
#types <- rw.clim.df.narm$Comp
#years <- toplot$Year
#names <- toplot$Tree
#data <- toplot$value
#types <- toplot$Comp

sparkplots <- function(years, names, data, types, legend=T){
  #makes stacked sparkline figures ala Tufte
  # stacks vertically, layering by "type"
  # legend appears at top right
  name.un <- unique(names)
  type.un <- unique(types)
  year <- as.numeric(as.character(years))
  quartz(width=5, height=6.5)
  par(oma=c(1,1,1,7), mar=c(1,1,1,1))
  plot(1,type="n",axes=F,ann=F, xlim=c(min(year),max(year)),ylim=c(0,length(name.un)/length(type.un)), yaxs="i", xaxs="i")   #ensure axes go to the edge of the data points (remove gap between 0 and x and y axes)
  axis(1)
for(j in 1:length(type.un)){
  t <- type.un[j]
namej <- names[types==t]
namej.un <- unique(namej)  #get the unique vector of names that are type t
  for(i in 1:length(namej.un)){
    n <- namej.un[i] #store the name of desired tree we're looping through
    temp <- data[which(names==n)]
    tempsc <- temp/max(temp)# - min(temp)) #/max(temp-min(temp)) # making values range from 0 to 1
    lines(x=year[which(names==n)],y=(tempsc+i-1), lty=j, lwd=1.5)
    if (j==1){
    text (max(year), y = i-0.5, n, pos=4, xpd=NA )
#     if(j==1 & i==1 & legend==T) 
#       legend(x=max(year)+(max(year)-min(year))/6 # put the legend 1/6 of the way beyond the plot
#              , y = length(name.un)/length(type.un)
#              , legend = type.un,box.lwd=0
#              , lty = 1:length(type.un), lwd = 1.5, xpd=NA) #get the legend outside the plot, by about 1/4 the width of the plot
#       
    }
}}}




#######################################
# plotting figures for R graphics
###### Not part of the function #########
# 
# 
# quartz(width=10, height=6.5)
# par(oma=c(1,1,1,1), mar=c(1,1,3,3), mfrow=c(1,3))
# 
# toplot <- subset(x=rw.clim.allelev,subset=Elev=="L")
# main <- "Low elevation"
# #toplot <- toplot[order(toplot$Tree),]
# 
# 
# years <- toplot$Year
# names <- toplot$Tree
# data <- toplot$value
# #types <- toplot$Comp
# types=rep(x=1,times=nrow(toplot))
# comps <- unique(toplot$Comp)
# 
# name.un <- unique(names)
# name.un <- name.un[order(name.un)]
# type.un <- unique(types)
# year <- as.numeric(as.character(years))
# 
# #---------- Low Elevation Panel -----------
# 
# plot(1,type="n",axes=F,ann=F, xlim=c(min(year),max(year)),ylim=c(0,length(name.un)/length(type.un)), yaxs="i", xaxs="i", main=main)   #ensure axes go to the edge of the data points (remove gap between 0 and x and y axes)
# axis(1)
# #for(j in 1:length(type.un)){
# #  t <- type.un[j]
# #  namej <- names[types==t]
# #  namej.un <- unique(namej)  #get the unique vector of names that are type t
#   for(i in 1:length(name.un)){
#     n <- name.un[i] #store the name of desired tree we're looping through
#     temp <- data[which(names==n)]
#     Comp <- toplot$Comp[which(names==n)]
#     tempsc <- temp/max(temp)# - min(temp)) #/max(temp-min(temp)) # making values range from 0 to 1
#     lines(x=year[which(names==n)],y=(tempsc+i-1), lty=as.numeric(Comp[1]), lwd=1.5)
# #    if (j==1){
#       text (max(year), y = i-0.5, n, pos=4, xpd=NA )
#       #     if(j==1 & i==1 & legend==T) 
#       #       legend(x=max(year)+(max(year)-min(year))/6 # put the legend 1/6 of the way beyond the plot
#       #              , y = length(name.un)/length(type.un)
#       #              , legend = type.un,box.lwd=0
#       #              , lty = 1:length(type.un), lwd = 1.5, xpd=NA) #get the legend outside the plot, by about 1/4 the width of the plot
#       #       
#     }
#   #}
# #}
# polygon(x=c(2000.5,2000.5,2003.5,2003.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000015",lwd=.001)
# #polygon(x=c(1975.5,1975.5,1978.5,1978.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000020",lwd=.001)
# mtext(text=main, side=3, line=1.8, cex=1.2, adj=.55)
# 
# 
# #------------------ Mid Elevation Panel ---------
# 
# toplot <- subset(x=rw.clim.allelev,subset=Elev=="M")
# main <- "Mid elevation"
# years <- toplot$Year
# names <- toplot$Tree
# data <- toplot$value
# #types <- toplot$Comp
# types=rep(x=1,times=nrow(toplot))
# comps <- unique(toplot$Comp)
# name.un <- unique(names)
# name.un <- name.un[order(name.un)]
# type.un <- unique(types)
# year <- as.numeric(as.character(years))
# plot(1,type="n",axes=F,ann=F, xlim=c(min(year),max(year)),ylim=c(0,length(name.un)/length(type.un)), yaxs="i", xaxs="i", main=main)   #ensure axes go to the edge of the data points (remove gap between 0 and x and y axes)
# axis(1)
# for(i in 1:length(name.un)){
#   n <- name.un[i] #store the name of desired tree we're looping through
#   temp <- data[which(names==n)]
#   Comp <- toplot$Comp[which(names==n)]
#   tempsc <- temp/max(temp)# - min(temp)) #/max(temp-min(temp)) # making values range from 0 to 1
#   lines(x=year[which(names==n)],y=(tempsc+i-1), lty=as.numeric(Comp[1]), lwd=1.5)
#   #    if (j==1){
#   text (max(year), y = i-0.5, n, pos=4, xpd=NA )     
# }
# polygon(x=c(2000.5,2000.5,2003.5,2003.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000015",lwd=.001)
# #polygon(x=c(1975.5,1975.5,1978.5,1978.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000020",lwd=.001)
# mtext(text=main, side=3, line=1.8, cex=1.2, adj=.55)
# 
# #-------------------- High Elevation Panel
# 
# toplot <- subset(x=rw.clim.allelev,subset=Elev=="H")
# main <- "High elevation"
# years <- toplot$Year
# names <- toplot$Tree
# data <- toplot$value
# #types <- toplot$Comp
# types=rep(x=1,times=nrow(toplot))
# comps <- unique(toplot$Comp)
# name.un <- unique(names)
# name.un <- name.un[order(name.un)]
# type.un <- unique(types)
# year <- as.numeric(as.character(years))
# plot(1,type="n",axes=F,ann=F, xlim=c(min(year),max(year)),ylim=c(0,length(name.un)/length(type.un)), yaxs="i", xaxs="i", main=main)   #ensure axes go to the edge of the data points (remove gap between 0 and x and y axes)
# axis(1)
# for(i in 1:length(name.un)){
#   n <- name.un[i] #store the name of desired tree we're looping through
#   temp <- data[which(names==n)]
#   Comp <- toplot$Comp[which(names==n)]
#   tempsc <- temp/max(temp)# - min(temp)) #/max(temp-min(temp)) # making values range from 0 to 1
#   lines(x=year[which(names==n)],y=(tempsc+i-1), lty=as.numeric(Comp[1]), lwd=1.5)
#   #    if (j==1){
#   text (max(year), y = i-0.5, n, pos=4, xpd=NA )     
# }
# polygon(x=c(2000.5,2000.5,2003.5,2003.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000015",lwd=.001)
# #polygon(x=c(1975.5,1975.5,1978.5,1978.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000020",lwd=.001)
# mtext(text=main, side=3, line=1.8, cex=1.2, adj=.55)
# 
# 
# 
# #####################################################################
# ############### Without competition showing as differeing line type
# ###################### Code for Final Figure 3 of R Graphics class ##
# quartz(width=10, height=6.5)
# par(oma=c(1,1,1,1), mar=c(1,1,3,1), mfrow=c(1,3))
# 
# toplot <- subset(x=rw.clim.allelev,subset=Elev=="L")
# main <- "Low elevation"
# #toplot <- toplot[order(toplot$Tree),]
# 
# 
# years <- toplot$Year
# names <- toplot$Tree
# data <- toplot$value
# #types <- toplot$Comp
# types=rep(x=1,times=nrow(toplot))
# comps <- unique(toplot$Comp)
# 
# name.un <- unique(names)
# name.un <- name.un[order(name.un)]
# type.un <- unique(types)
# year <- as.numeric(as.character(years))
# 
# 
# plot(1,type="n",axes=F,ann=F, xlim=c(min(year),max(year)),ylim=c(0,length(name.un)/length(type.un)), yaxs="i", xaxs="i", main=main)   #ensure axes go to the edge of the data points (remove gap between 0 and x and y axes)
# axis(1)
# #for(j in 1:length(type.un)){
# #  t <- type.un[j]
# #  namej <- names[types==t]
# #  namej.un <- unique(namej)  #get the unique vector of names that are type t
# for(i in 1:length(name.un)){
#   n <- name.un[i] #store the name of desired tree we're looping through
#   temp <- data[which(names==n)]
#   Comp <- toplot$Comp[which(names==n)]
#   tempsc <- temp/max(temp)# - min(temp)) #/max(temp-min(temp)) # making values range from 0 to 1
#   lines(x=year[which(names==n)],y=(tempsc+i-1), lwd=1.5)
#   #    if (j==1){
# #text (max(year), y = i-0.5, n, pos=4, xpd=NA )
#   #     if(j==1 & i==1 & legend==T) 
#   #       legend(x=max(year)+(max(year)-min(year))/6 # put the legend 1/6 of the way beyond the plot
#   #              , y = length(name.un)/length(type.un)
#   #              , legend = type.un,box.lwd=0
#   #              , lty = 1:length(type.un), lwd = 1.5, xpd=NA) #get the legend outside the plot, by about 1/4 the width of the plot
#   #       
# }
# #}
# #}
# #polygon(x=c(2000.5,2000.5,2003.5,2003.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000015", border="#00000015")
# #polygon(x=c(1975.5,1975.5,1978.5,1978.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000020",lwd=.001)
# mtext(text=main, side=3, line=1.8, cex=1.2, adj=.55)
# 
# 
# #------------------
# 
# toplot <- subset(x=rw.clim.allelev,subset=Elev=="M")
# main <- "Mid elevation"
# years <- toplot$Year
# names <- toplot$Tree
# data <- toplot$value
# #types <- toplot$Comp
# types=rep(x=1,times=nrow(toplot))
# comps <- unique(toplot$Comp)
# name.un <- unique(names)
# name.un <- name.un[order(name.un)]
# type.un <- unique(types)
# year <- as.numeric(as.character(years))
# plot(1,type="n",axes=F,ann=F, xlim=c(min(year),max(year)),ylim=c(0,length(name.un)/length(type.un)), yaxs="i", xaxs="i", main=main)   #ensure axes go to the edge of the data points (remove gap between 0 and x and y axes)
# axis(1)
# for(i in 1:length(name.un)){
#   n <- name.un[i] #store the name of desired tree we're looping through
#   temp <- data[which(names==n)]
#   Comp <- toplot$Comp[which(names==n)]
#   tempsc <- temp/max(temp)# - min(temp)) #/max(temp-min(temp)) # making values range from 0 to 1
#   lines(x=year[which(names==n)],y=(tempsc+i-1), lwd=1.5)
#   #text (max(year), y = i-0.5, n, pos=4, xpd=NA )     
# }
# #polygon(x=c(2000.5,2000.5,2003.5,2003.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000015",border="#00000015")
# #polygon(x=c(1975.5,1975.5,1978.5,1978.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000020",lwd=.001)
# mtext(text=main, side=3, line=1.8, cex=1.2, adj=.55)
# 
# #--------------------
# 
# toplot <- subset(x=rw.clim.allelev,subset=Elev=="H")
# # can also knock of 11 and 12 to make each elevation have the same number of trees
# toplot <- toplot[which(toplot$Tree != "H.11H" & toplot$Tree != "H.11L" & toplot$Tree != "H.12H" & toplot$Tree != "H.12L"),]
# main <- "High elevation"
# years <- toplot$Year
# names <- toplot$Tree
# data <- toplot$value
# #types <- toplot$Comp
# types=rep(x=1,times=nrow(toplot))
# comps <- unique(toplot$Comp)
# name.un <- unique(names)
# name.un <- name.un[order(name.un)]
# type.un <- unique(types)
# year <- as.numeric(as.character(years))
# plot(1,type="n",axes=F,ann=F, xlim=c(min(year),max(year)),ylim=c(0,length(name.un)/length(type.un)), yaxs="i", xaxs="i", main=main)   #ensure axes go to the edge of the data points (remove gap between 0 and x and y axes)
# axis(1)
# for(i in 1:length(name.un)){
#   n <- name.un[i] #store the name of desired tree we're looping through
#   temp <- data[which(names==n)]
#   Comp <- toplot$Comp[which(names==n)]
#   tempsc <- temp/max(temp)# - min(temp)) #/max(temp-min(temp)) # making values range from min/max to 1
#   lines(x=year[which(names==n)],y=(tempsc+i-1), lwd=1.5)
#   #text (max(year), y = i-0.5, n, pos=4, xpd=NA )     
# }
# #polygon(x=c(2000.5,2000.5,2003.5,2003.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000015", border="#00000015")
# #polygon(x=c(1975.5,1975.5,1978.5,1978.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000020",lwd=.001)
# mtext(text=main, side=3, line=1.8, cex=1.2, adj=.55)
# 
# 
# 
# 
# 
# #######################################################################
# ############### Trying to plot masters with variance behind them ######
# #######################################################################
# mat <- matrix(c(1,1,1,
#                 2,3,4),nrow=2,ncol=3, byrow=T)
# 
# quartz(width=8, height=6)
# par(mar=c(3,3,1,2), oma=c(2,3,1,1))
# layout(mat=mat, heights=c(.6,1,1,1))
# layout.show(4)
# linecol <- "#AAAAAA"
# plot(POTR.L~rownames(POTR.master), data=POTR.master, type="n", lwd=2, col="red3", ylab="", xlab="", ylim=c(-.2, 4.7), xlim=c(1900,2013), yaxt="n", mgp = c(2,.7,0), cex.axis=1.2)
# #legend("bottomleft", legend=c("Low", "Mid", "High", "Individual"), lwd=c(2,2,2,1), col= c("red3", "purple3", "blue3", "#999999"), ncol=4, bty="n")
# mtext(text="Detrended Master \n Chronologies",side=2, line=1.4)
# text(x=c(2011,2011,2011), y=c(1.15,2.65,4.15), pos=4, labels=c("Low", "Mid", "High"), font=2)
# abline(h=1, lty=2, col="gray")
# abline(h=2.5, lty=2, col="gray")
# abline(h=4, lty=2, col="gray")
# 
# 
# #------ adding individual chronologies under masters -----------
# toplot <- subset(x=rw.clim.allelev,subset=Elev=="L")
# years <- toplot$Year
# names <- toplot$Tree
# data <- toplot$value
# #types <- toplot$Comp
# types=rep(x=1,times=nrow(toplot))
# comps <- unique(toplot$Comp)
# 
# name.un <- unique(names)
# name.un <- name.un[order(name.un)]
# type.un <- unique(types)
# year <- as.numeric(as.character(years))
# 
# for(i in 1:length(name.un)){
#   n <- name.un[i] #store the name of desired tree we're looping through
#   temp <- data[which(names==n)]
#   Comp <- toplot$Comp[which(names==n)]
#   #tempsc <- temp/max(temp)# - min(temp)) #/max(temp-min(temp)) # making values range from 0 to 1
#   lines(x=year[which(names==n)],y=(temp), lwd=1, col=linecol)
# }
# points(PIPO.L~X, data=PIPO.master, type="l", lwd=2, col="red3")#, ylab="Detrended Master Chronologies", xlab="Year", ylim=c(0, 4.5), yaxt="n")
# #  text (max(year), y = i-0.5, n, pos=4, xpd=NA )
# 
# toplot <- subset(x=rw.clim.allelev,subset=Elev=="M")
# years <- toplot$Year
# names <- toplot$Tree
# data <- toplot$value
# #types <- toplot$Comp
# types=rep(x=1,times=nrow(toplot))
# comps <- unique(toplot$Comp)
# 
# name.un <- unique(names)
# name.un <- name.un[order(name.un)]
# type.un <- unique(types)
# year <- as.numeric(as.character(years))
# 
# for(i in 1:length(name.un)){
#   n <- name.un[i] #store the name of desired tree we're looping through
#   temp <- data[which(names==n)]
#   Comp <- toplot$Comp[which(names==n)]
#   #tempsc <- temp/max(temp)# - min(temp)) #/max(temp-min(temp)) # making values range from 0 to 1
#   lines(x=year[which(names==n)],y=(temp + 1.5), lwd=1, col=linecol)
# }
# 
# toplot <- subset(x=rw.clim.allelev,subset=Elev=="H")
# years <- toplot$Year
# names <- toplot$Tree
# data <- toplot$value
# #types <- toplot$Comp
# types=rep(x=1,times=nrow(toplot))
# comps <- unique(toplot$Comp)
# 
# name.un <- unique(names)
# name.un <- name.un[order(name.un)]
# type.un <- unique(types)
# year <- as.numeric(as.character(years))
# 
# for(i in 1:length(name.un)){
#   n <- name.un[i] #store the name of desired tree we're looping through
#   temp <- data[which(names==n)]
#   Comp <- toplot$Comp[which(names==n)]
#   #tempsc <- temp/max(temp)# - min(temp)) #/max(temp-min(temp)) # making values range from 0 to 1
#   lines(x=year[which(names==n)],y=(temp + 3), lwd=1, col=linecol)
# }
# points(PIPO.M+1.5~X, data=PIPO.master, type="l", lwd=2, col="purple3")
# points(PIPO.H+3~X, data=PIPO.master, type="l", lwd=2, col="blue3")
# 
# 
# 
# 
# # ---------- now making sparkplots below as figs 2:4 --------
# 
# par(mar=c(1,0,2,1))
# toplot <- subset(x=rw.clim.allelev,subset=Elev=="L")
# main <- "Low elevation"
# #toplot <- toplot[order(toplot$Tree),]
# 
# 
# years <- toplot$Year
# names <- toplot$Tree
# data <- toplot$value
# #types <- toplot$Comp
# types=rep(x=1,times=nrow(toplot))
# comps <- unique(toplot$Comp)
# 
# name.un <- unique(names)
# name.un <- name.un[order(name.un)]
# type.un <- unique(types)
# year <- as.numeric(as.character(years))
# 
# 
# plot(1,type="n",axes=F,ann=F, xlim=c(min(year),max(year)),ylim=c(0,length(name.un)/length(type.un)), yaxs="i", xaxs="i", main=main)   #ensure axes go to the edge of the data points (remove gap between 0 and x and y axes)
# axis(1)
# #for(j in 1:length(type.un)){
# #  t <- type.un[j]
# #  namej <- names[types==t]
# #  namej.un <- unique(namej)  #get the unique vector of names that are type t
# for(i in 1:length(name.un)){
#   n <- name.un[i] #store the name of desired tree we're looping through
#   temp <- data[which(names==n)]
#   Comp <- toplot$Comp[which(names==n)]
#   tempsc <- temp/max(temp)# - min(temp)) #/max(temp-min(temp)) # making values range from 0 to 1
#   lines(x=year[which(names==n)],y=(tempsc+i-1), lwd=1.5)
# }
#  mtext(text=main, side=3, line=.4, cex=1, adj=.55)
# 
# 
# #------------------
# 
# toplot <- subset(x=rw.clim.allelev,subset=Elev=="M")
# main <- "Mid elevation"
# years <- toplot$Year
# names <- toplot$Tree
# data <- toplot$value
# #types <- toplot$Comp
# types=rep(x=1,times=nrow(toplot))
# comps <- unique(toplot$Comp)
# name.un <- unique(names)
# name.un <- name.un[order(name.un)]
# type.un <- unique(types)
# year <- as.numeric(as.character(years))
# plot(1,type="n",axes=F,ann=F, xlim=c(min(year),max(year)),ylim=c(0,length(name.un)/length(type.un)), yaxs="i", xaxs="i", main=main)   #ensure axes go to the edge of the data points (remove gap between 0 and x and y axes)
# axis(1)
# for(i in 1:length(name.un)){
#   n <- name.un[i] #store the name of desired tree we're looping through
#   temp <- data[which(names==n)]
#   Comp <- toplot$Comp[which(names==n)]
#   tempsc <- temp/max(temp)# - min(temp)) #/max(temp-min(temp)) # making values range from 0 to 1
#   lines(x=year[which(names==n)],y=(tempsc+i-1), lwd=1.5)
#   #text (max(year), y = i-0.5, n, pos=4, xpd=NA )     
# }
# #polygon(x=c(2000.5,2000.5,2003.5,2003.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000015",border="#00000015")
# #polygon(x=c(1975.5,1975.5,1978.5,1978.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000020",lwd=.001)
# mtext(text=main, side=3, line=.4, cex=1, adj=.55)
# 
# #--------------------
# 
# toplot <- subset(x=rw.clim.allelev,subset=Elev=="H")
# # can also knock of 11 and 12 to make each elevation have the same number of trees
# toplot <- toplot[which(toplot$Tree != "H.11H" & toplot$Tree != "H.11L" & toplot$Tree != "H.12H" & toplot$Tree != "H.12L"),]
# main <- "High elevation"
# years <- toplot$Year
# names <- toplot$Tree
# data <- toplot$value
# #types <- toplot$Comp
# types=rep(x=1,times=nrow(toplot))
# comps <- unique(toplot$Comp)
# name.un <- unique(names)
# name.un <- name.un[order(name.un)]
# type.un <- unique(types)
# year <- as.numeric(as.character(years))
# plot(1,type="n",axes=F,ann=F, xlim=c(min(year),max(year)),ylim=c(0,length(name.un)/length(type.un)), yaxs="i", xaxs="i", main=main)   #ensure axes go to the edge of the data points (remove gap between 0 and x and y axes)
# axis(1)
# for(i in 1:length(name.un)){
#   n <- name.un[i] #store the name of desired tree we're looping through
#   temp <- data[which(names==n)]
#   Comp <- toplot$Comp[which(names==n)]
#   tempsc <- temp/max(temp)# - min(temp)) #/max(temp-min(temp)) # making values range from min/max to 1
#   lines(x=year[which(names==n)],y=(tempsc+i-1), lwd=1.5)
#   #text (max(year), y = i-0.5, n, pos=4, xpd=NA )     
# }
# #polygon(x=c(2000.5,2000.5,2003.5,2003.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000015", border="#00000015")
# #polygon(x=c(1975.5,1975.5,1978.5,1978.5), y=c(length(name.un)/length(type.un)+.5,0,0,length(name.un)/length(type.un)+.5), col="#00000020",lwd=.001)
# mtext(text=main, side=3, line=.4, cex=1, adj=.55)
# 
# 
# 
