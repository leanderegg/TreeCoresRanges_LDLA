...
#xtabs(~Species + Band + Comp, MTtreesnew)
# good. all trees seem to be there.



#=========== Using new, clean, and complete Dataz ===================
boxplot(DBH~Band+Species, data=MTtreesnew)
# supports what I was thinking for the DBH trends above
boxplot(BA_tot~Comp+Band+Species, data=MTtreesnew, col=c("gray", "white"))

levels(MTtreesnew$Band) <- list(L = "L", M="M", H="H") # reordering the levels so they plot low to high
levels(MTtreesnew$Species) <- list(TSHE="TSHE", PSME="PSME", ABLA="ABLA")
quartz(width=7, height=5)
par(mfrow=c(1,3),mar=c(5,5,3,0))
for(i in 1:3){
  sp <- levels(MTtreesnew$Species)[i]
  boxplot(BA_tot~Comp + Band, data=MTtreesnew[which(MTtreesnew$Species==sp),], main= sp, col=c("grey", "white"), ylim=c(0,350), ylab=expression(paste("Stand Basal Area (", ft^2, ha^-1, ")")))
}

bandtest <- MTtreesnew$Band
...
# because the high and low treatments pretty much vary between elevations for ABLA and a little for TSHE
...
for(i in 1:3){
  sp <- levels(MTtreesnew$Species)[i]
  boxplot(BA_totR~ Band, data=MTtreesnew[which(MTtreesnew$Species==sp),], 
...
mort <- MTtreesnew$BA_sameRG/MTtreesnew$BA_same_all # getting percentage dead of total same basal area
mort[which(is.na(mort))] <- 0
perc_mort <- tapply(mort, INDEX=list(MTtreesnew$Species, MTtreesnew$Band), FUN=mean)
...
BAsame <- tapply(MTtreesnew$BA_same_all, INDEX=list(MTtreesnew$Species, MTtreesnew$Band), FUN=mean)
...
mspLreg <- rbind(regLtmp[[1]], regLtmp[[2]], regLtmp[[3]]) # put it into rows = L, M, Hcolumns=regen classes
...
legend("top", legend=c("Live", "Dead"), fill=c(paste(cols[3], 22, sep=""), "black"), border=cols[3], bty="n", cex=.8)
